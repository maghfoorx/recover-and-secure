...
export class PosPrinter {
  ...
  public static print(
    ...
    return new Promise((resolve, reject) => {
      ...
      // console.log(options.pathTemplate, "pathTOTEMPLATE PROP");
      // mainWindow.loadFile(
      //   options.pathTemplate || join(__dirname, "renderer/index.html"),
      // );

      // this is some really disgusting code to print receipts at the moment.
      // I'm not able to send messages back and forth between the main and renderer processes.
      // So temporarily just creating a string html and loading that on the new window.
      // I hope it works for the printer.
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (const [lineIndex, line] of data.entries()) {
        ...
        if (line.type === "image" && !line.path && !line.url) {
          ...
          reject(
            new Error(
              "An Image url/path is required for type image",
            ).toString(),
          );
          break;
        }
